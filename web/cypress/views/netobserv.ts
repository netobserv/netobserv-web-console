...
export const Operator = {
    ...
    deployFlowcollectorFromUI: () => {
        ...
        // Processing tab
        cy.get(pluginSelectors.privilegedToggle).should('exist').check({ force: true })
        // Enable PacketDrop
            ...
            cy.adminCLI('oc get flowcollector cluster -o name', {
                ...
                cy.get('body').then($body => {
                    const errors = $body.find(
                        '.pf-v6-c-alert__title, .pf-v6-c-helper-text__item-text.pf-m-error, ' +
                        '.pf-v5-c-alert__title, .pf-v5-c-helper-text__item-text.pf-m-error'
                    )
                    if (errors.length > 0) {
